  ...
  return (
    <div>
      {/* <Paper sx={{ width: "100%", overflow: "hidden" }}> */}
        <TableContainer
          component={Paper}
          className="border border-slate-200 rounded-xl px-5 bg-[#fbfbfc] w-full overflow-hidden 2xl:overflow-auto"
        >
                ...
                {table.table_head.map(
                  (data: TableContentInterface, i: number) => {
                    return (
                      <TableCell
                        key={i}
                        className="!font-poppins !text-lg !p-8 !text-black !font-medium"
                      >
                        {data.title}
                      </TableCell>
                    );
              ...
              {filterData &&
                filterData.map((data: TableContentInterface, i: number) => {
                  return (
                    <TableRow
                      key={i}
                      className="cursor-pointer hover:bg-[#E6F8F9] text-lg"
                    >
                      <TableCell>

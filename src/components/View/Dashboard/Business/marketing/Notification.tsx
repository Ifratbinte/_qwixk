...
const Notification = () => {
  const table = table_data.marketing.notification;
  ...
  useEffect(() => {
    setFilterData(
      table.table_content.filter((item: TableContentInterface, index: number) => {
  ...
  return (
    <div>
      ...
      >
        <Table sx={{ minWidth: 650 }} className="overflow-x-scroll">
          <TableHead>
            <TableRow>
              {table.table_head.map((data: TableContentInterface, i: number) => {
                return (
                  <TableCell
                    key={i}
          ...
          <TableBody>
            {table.table_content.map((data: TableContentInterface, i: number) => {
              return (
                <TableRow key={i} className="cursor-pointer hover:bg-[#E6F8F9]">
                  ...
                  <TableCell className="">{data.start}</TableCell>
